    ...
    <div className="w-full">
      ...
      <div className="flex flex-wrap justify-center gap-4 mb-12">
        <button
          ...
          className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-bold transition-all shadow-md ${activeTab === 'rug' ? 'bg-[#3AC2FE] text-white scale-105 shadow-xl' : 'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10'}`}
        >
          <Sparkles size={20} /> Szőnyegtisztítás
        </button>
        <button
          onClick={() => setActiveTab('uph')}
          className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-bold transition-all shadow-md ${activeTab === 'uph' ? 'bg-[#3AC2FE] text-white scale-105 shadow-xl' : 'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10'}`}
        >
          <Sofa size={20} /> Kárpittisztítás
        </button>
        <button
          onClick={() => setActiveTab('car')}
          className={`flex items-center gap-3 px-8 py-4 rounded-2xl font-bold transition-all shadow-md ${activeTab === 'car' ? 'bg-[#3AC2FE] text-white scale-105 shadow-xl' : 'bg-white/5 text-gray-400 hover:bg-white/10 border border-white/10'}`}
        >
            ...
            <div className="bg-white/5 rounded-3xl p-8 border border-white/10">
              ...
              <div className="space-y-4">
                {Object.entries(pricingUph?.types || {}).map(([name, price]) => (
                  <div key={name} className="flex justify-between items-center border-b border-white/5 pb-3">
                    <span className="text-lg font-medium">{name}</span>
                    <span className="text-xl font-black text-white">{formatPrice(price)} Ft <span className="text-xs text-gray-400">/db-tól</span></span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {Object.entries(pricingCar?.packages || {}).map(([name, data]: [string, any]) => {
              ...
              return (
                <div key={name} className="bg-white/5 rounded-3xl p-8 border border-white/10 flex flex-col relative overflow-hidden group hover:border-[#3AC2FE]/50 transition-all">
                  ...
                  <div className="text-3xl font-black text-[#3AC2FE] mb-8">
                  ...
                  <div className="space-y-3 flex-1">
                    {serviceList.map((s: string, idx: number) => (
                      <div key={idx} className="flex items-start gap-3 text-sm text-gray-300">
                        <Check size={16} className="text-[#3AC2FE] mt-0.5 flex-shrink-0" />
                        <span>{s}</span>
                      </div>
                    ))}
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {/* Méret felárak */}
            <div className="bg-white/5 rounded-3xl p-8 border border-white/10">
              <h4 className="text-xl font-bold text-[#3AC2FE] mb-6">Gépjármű Kategória felárak</h4>
              <div className="space-y-4">
            ...
            {/* Autó extrák */}
            <div className="bg-white/5 rounded-3xl p-8 border border-white/10">
              <h4 className="text-xl font-bold text-[#3AC2FE] mb-6">Kiegészítő Extrák</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
