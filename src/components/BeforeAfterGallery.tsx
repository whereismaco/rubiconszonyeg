      ...
      <div className="flex justify-center gap-4 overflow-x-auto pb-4 px-2 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden snap-x">
        {pairs.map((pair, idx) => (
          <button
            ...
            className={`relative shrink-0 snap-center w-20 h-20 md:w-24 md:h-24 rounded-xl overflow-hidden border-[3px] transition-all duration-300 ${activeIndex === idx ? 'border-[#3AC2FE] scale-105 shadow-xl' : 'border-transparent shadow-sm hover:border-gray-300 opacity-70 hover:opacity-100'}`}
          >
            <Image 
              src={pair.after} 
              alt={pair.name} 
